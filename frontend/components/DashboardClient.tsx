...
function StatCardSkeleton() {
  return (
    <div className="rounded-2xl border border-white/[0.05] bg-white/[0.02] p-5">
      ...
      <Shimmer className="h-8 w-16 mb-2 rounded-lg" />
...
function MapSkeleton() {
  return (
    <div className="h-[360px] rounded-none bg-white/[0.02] flex flex-col items-center justify-center gap-3 relative overflow-hidden">
      <div className="absolute inset-0 -translate-x-full animate-[shimmer_2s_infinite] bg-gradient-to-r from-transparent via-white/[0.03] to-transparent" />
      <div className="w-8 h-8 rounded-full border-2 border-[#1d6bf3]/30 border-t-[#1d6bf3]/80 animate-spin" />
      <span className="text-xs text-zinc-700 relative z-10">Loading map…</span>
    </div>
  );
}
...
export default function DashboardClient() {
  ...
  const stats = [
    { label: "Submissions",     value: insights?.total ?? 0,             icon: Users,         color: "#3b82f6" },
    ...
    { label: "Recommendations", value: recommendations.length,            icon: CheckCircle,   color: "#8b5cf6" },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#08080c]">
      ...
      <div className="pointer-events-none fixed top-0 left-1/2 -translate-x-1/2 w-[900px] h-[280px] bg-[#1d6bf3]/[0.055] blur-[110px] rounded-full" />

      {/* Nav */}
      <nav className="sticky top-0 z-50 border-b border-white/[0.05] bg-[#08080c]/85 backdrop-blur-xl px-6 py-3.5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3.5">
            <Link href="/" className="p-1.5 rounded-lg hover:bg-white/[0.06] text-zinc-600 hover:text-zinc-300 transition-all">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-zinc-100 text-sm tracking-tight">CivicPulse</span>
              <span className="text-zinc-800 text-sm">/</span>
              <span className="text-zinc-500 text-sm">MP Dashboard</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-zinc-700 hidden sm:block tabular-nums" suppressHydrationWarning>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {/* Stats row */}
        <motion.div variants={stagger} initial="hidden" animate="show" className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-7">
          {loading
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-start gap-2.5">
                  <MapPin className="w-4 h-4 text-[#1d6bf3] mt-0.5" />
                  <div>
                    <h2 className="text-sm font-semibold text-zinc-100">Demand Hotspot Map</h2>
                    <p className="text-xs text-zinc-600 mt-0.5">Issue density by ward — circle size reflects volume</p>
                  </div>
                </div>
                <span className="text-xs text-zinc-600 bg-white/[0.04] border border-white/[0.06] px-2.5 py-1 rounded-full">
                  {insights.wardHotspots.length} active wards
                </span>
              </div>
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <Panel>
              ...
              {loading ? (
                ...
                <EmptyState icon={<BarChart3 className="w-5 h-5" />} title="No clusters yet" message="Submit citizen feedback to see issue clusters." />
              ) : (
            ...
            <Panel>
              ...
              {loading ? (
                ...
                <EmptyState icon={<Users className="w-5 h-5" />} title="No submissions yet" message="Citizens haven't submitted any feedback yet." />
              ) : (
                <motion.div variants={stagger} initial="hidden" animate="show">
          ...
          <div className="space-y-5">
            ...
            <Panel>
              <div className="flex items-start justify-between mb-5">
                ...
                <motion.button
                  ...
                  className="flex items-center gap-1.5 text-xs bg-[#1d6bf3] hover:bg-[#2b78ff] disabled:opacity-40 disabled:cursor-not-allowed text-white px-3 py-1.5 rounded-lg transition-all shadow-md shadow-[#1d6bf3]/20"
                >
                  {genLoading
              ...
              {loading ? (
                ...
                <EmptyState icon={<Sparkles className="w-5 h-5" />} title="No recommendations" message="Click Generate to create AI-ranked development priorities." />
              ) : (
            ...
            <Panel>
              <SectionHeader
                ...
                icon={<BarChart3 className="w-4 h-4 text-zinc-600" />}
              />
              {loading ? (
                ...
                <EmptyState icon={<BarChart3 className="w-5 h-5" />} title="No data" message="Category data appears after submissions." />
              ) : (
            ...
            {/* Ward hotspots list */}
            {(loading || (insights?.wardHotspots && insights.wardHotspots.length > 0)) && (
              <Panel>
                <SectionHeader
                  title="Top Wards"
                  subtitle="Most reported areas"
                  icon={<MapPin className="w-4 h-4 text-zinc-600" />}
                />
                {loading ? (
...
function SectionHeader({ title, subtitle, badge, icon }: {
  ...
  return (
    <div className="flex items-start justify-between mb-4">
      ...
      {badge && (
        <span className="text-xs text-zinc-600 bg-white/[0.04] border border-white/[0.06] px-2.5 py-1 rounded-full shrink-0">
          {badge}
        </span>
      )}
    </div>
...
function StatCard({ label, value, icon: Icon, color }: {
  ...
  return (
    <motion.div
      variants={fadeUp}
      whileHover={{ scale: 1.015, borderColor: "rgba(255,255,255,0.10)" }}
      className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 cursor-default transition-colors duration-200"
    >
      <div className="flex items-start justify-between mb-3.5">
        <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${color}16` }}>
          <Icon className="w-4 h-4" style={{ color }} />
        </div>
        <ArrowUpRight className="w-3.5 h-3.5 text-zinc-800" />
      </div>
      <p className="text-[2rem] font-bold text-zinc-100 tabular-nums tracking-tight leading-none mb-1.5">
...
function ClusterRow({ cluster, rank }: { cluster: Cluster; rank: number }) {
  const color = CATEGORY_COLORS[cluster.category] || "#3b82f6";
  const pct   = Math.min(100, (cluster.priority_score / 10) * 100);
  return (
    <motion.div
      variants={fadeUp}
      whileHover={{ backgroundColor: "rgba(255,255,255,0.02)" }}
      className="flex items-center gap-3 py-3 border-b border-white/[0.04] last:border-0 rounded-lg px-1.5 -mx-1.5 transition-colors duration-150 cursor-default"
    >
      <span className="text-[11px] font-mono text-zinc-700 w-4 text-right shrink-0">{rank}</span>
      <div className="w-1 h-7 rounded-full shrink-0" style={{ backgroundColor: color, opacity: 0.85 }} />
      <div className="flex-1 min-w-0">
        <p className="text-[13px] text-zinc-200 truncate font-medium leading-snug">{cluster.theme}</p>
        <div className="flex items-center gap-2 mt-1.5">
          <div className="h-1 flex-1 max-w-[72px] bg-white/[0.06] rounded-full overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${pct}%` }}
              transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] as [number, number, number, number], delay: rank * 0.04 }}
              className="h-full rounded-full"
              style={{ backgroundColor: color }}
            />
          </div>
          <span className="text-[11px] text-zinc-700 tabular-nums">{cluster.priority_score.toFixed(1)}</span>
        </div>
      </div>
...
function SubmissionRow({ submission }: { submission: Submission }) {
  ...
  return (
    <motion.div
      variants={fadeUp}
      whileHover={{ backgroundColor: "rgba(255,255,255,0.02)" }}
      className="flex items-start gap-3 py-3 border-b border-white/[0.04] last:border-0 rounded-lg px-1.5 -mx-1.5 transition-colors duration-150 cursor-default"
    >
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1.5">
          ...
          <span className="text-[11px] text-zinc-600">{submission.category}</span>
        </div>
      </div>
...
function RecommendationCard({ rec, rank }: { rec: Recommendation; rank: number }) {
  const color = CATEGORY_COLORS[rec.category] || "#3b82f6";
  const score = Math.min(100, Math.round(rec.priority_score));
  return (
    <motion.div
      variants={fadeUp}
      whileHover={{ borderColor: "rgba(255,255,255,0.09)", backgroundColor: "rgba(255,255,255,0.022)" }}
      className="rounded-xl border border-white/[0.06] bg-white/[0.01] p-4 transition-all duration-200 cursor-default"
    >
      {/* Rank + score */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-mono text-zinc-700">#{rank}</span>
          <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: color }} />
          <span className="text-[10px] text-zinc-600 uppercase tracking-wide">{rec.category}</span>
        </div>
        {/* Score pill */}
        <div className="flex items-center gap-1.5">
          <div className="h-1 w-12 bg-white/[0.06] rounded-full overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${score}%` }}
              transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] as [number, number, number, number], delay: rank * 0.1 }}
              className="h-full rounded-full"
              style={{ backgroundColor: color }}
            />
          </div>
      ...
      {/* Title */}
      <p className="text-[13px] font-semibold text-zinc-100 mb-1.5 leading-snug">{rec.title}</p>
      <p className="text-[12px] text-zinc-600 mb-3 line-clamp-2 leading-relaxed">{rec.description}</p>

      {/* Meta */}
      <div className="flex items-center gap-3 text-[11px] text-zinc-700 mb-2.5">
      ...
      {rec.estimated_impact && (
        <div className="rounded-lg bg-[#00d4aa]/[0.07] border border-[#00d4aa]/20 px-3 py-2 mb-2.5">
          <p className="text-[11px] text-[#00d4aa] flex items-start gap-1.5 leading-relaxed">
      ...
      {/* Rationale */}
      {rec.rationale && (
        <p className="text-[11px] text-zinc-700 flex items-start gap-1 border-t border-white/[0.04] pt-2.5 leading-relaxed">
          <ChevronRight className="w-3 h-3 shrink-0 mt-0.5 text-[#1d6bf3]/60" />
          {rec.rationale}
        </p>
      )}
    </motion.div>
  ...
  return (
    <div className="group cursor-default">
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-[12px] text-zinc-500 group-hover:text-zinc-300 transition-colors truncate mr-2">{name}</span>
        <span className="text-[11px] font-semibold text-zinc-600 tabular-nums shrink-0">{value}</span>
      </div>
...
function WardRow({ ward, rank, total }: { ward: { ward: string; count: number }; rank: number; total: number }) {
  ...
  return (
    <motion.div
      variants={fadeUp}
      whileHover={{ backgroundColor: "rgba(255,255,255,0.02)" }}
      className="flex items-center gap-3 py-2.5 border-b border-white/[0.04] last:border-0 rounded-lg px-1.5 -mx-1.5 transition-colors duration-150 cursor-default"
    >
      <span className="text-[11px] font-mono text-zinc-700 w-4 text-right shrink-0">{rank}</span>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[13px] text-zinc-400 truncate">{ward.ward}</span>
          <span className="text-[11px] text-zinc-600 tabular-nums shrink-0 ml-2">{ward.count}</span>
        </div>
        <div className="h-1 bg-white/[0.05] rounded-full overflow-hidden">
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: `${pct}%` }}
            transition={{ duration: 0.75, ease: [0.22, 1, 0.36, 1] as [number, number, number, number], delay: rank * 0.05 }}
...
function EmptyState({ icon, title, message }: { icon: React.ReactNode; title: string; message: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center">
      <div className="w-11 h-11 rounded-2xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-center text-zinc-700 mb-3">
        {icon}
      </div>
